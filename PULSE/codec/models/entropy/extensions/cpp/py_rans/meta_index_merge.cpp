...
std::vector<uint16_t> load_probabilities(
    const py::array_t<
        uint16_t,
    ...
    const Layout& layout)
{
    ...
    if (info.ndim != 1
        || static_cast<size_t>(info.shape[0])
            != layout.total_probabilities) {
        throw std::runtime_error(
            "invalid Meta Prior index-merge probability table shape");
    }
    const auto* input = static_cast<const uint16_t*>(info.ptr);
    std::vector<uint16_t> probabilities(
        input,
        input + layout.total_probabilities);
    for (const uint16_t probability : probabilities) {
        if (probability == 0 || probability >= kProbabilityTotal) {
            throw std::runtime_error(
...
py::bytes encode_meta_prior_index_merge(
    const py::array_t<
        uint8_t,
        py::array::c_style | py::array::forcecast>& indexes,
    const int batch,
    const int height,
    const int width,
    ...
    const int adaptation_shift)
{
    ...
    const size_t count = static_cast<size_t>(index_info.size);
    validate_shape(batch, height, width, count);
    const auto* values = static_cast<const uint8_t*>(index_info.ptr);
