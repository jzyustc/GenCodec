                                   ...
                                   int64_t mask_width)
{
    ...
    TORCH_CHECK(sd.at("decoder.blocks.3.conv4.weight").sizes() ==
                    at::IntArrayRef({800,288,1,1}) &&
                sd.find("decoder.blocks.4.conv1.weight") == sd.end(),
                "PULSE-L requires four288-channel DCBs with FFN800");
                                               ...
                                               const at::Tensor& compact1_nhwc, int64_t qp) const
{
    TORCH_CHECK(z_i16.device().is_cpu() && z_i16.scalar_type() == at::kShort
                    ...
                    && z_i16.size(0) == 1 && z_i16.size(1) == 144
                    && z_i16.size(2)*4 == mask0_.size(2)
                    && z_i16.size(3)*4 == mask0_.size(3),
...
std::map<std::string,double> DecodeWrapperProxy::profile_compact(
    const at::Tensor& z_i16, const at::Tensor& compact0,
    const at::Tensor& compact1, int64_t qp) const {
    TORCH_CHECK(qp >= 0 && qp < qp_num_, "invalid QP");
    std::map<std::string,double> times;
    auto last=std::chrono::steady_clock::now();
    auto mark=[&](const std::string& name) {
        const auto now=std::chrono::steady_clock::now();
        times[name]=std::chrono::duration<double,std::milli>(now-last).count();
        last=now;
    };
    auto zhat=z_i16.to(at::kBFloat16).contiguous(at::MemoryFormat::ChannelsLast);
    auto a=expand_compact_checkerboard(compact0,0);
    auto b=expand_compact_checkerboard(compact1,1);
    mark("compact_conversion");
    auto params=hyper_dec_.forward(zhat,q_hyper_z_[qp],q_hyper_m_basic_[qp]);
    mark("hyper_decoder");
    auto common=y_prior_fusion_.forward(params,q_prior_[qp],a,mask0_);
    mark("entropy_step1");
    auto parts=common.chunk(3,0);
    auto z=y_spatial_prior_.forward(parts[2],b,parts[0],parts[1],q_prior_[qp],mask0_,codec_scale_[qp]);
    mark("entropy_step2");
    auto s=up0_.forward(z);
    mark("decoder_input_projection");
    for(int i=0;i<g_num_cond_blocks;++i) {
        s=blocks_[i].forward(s,dico_scale_[qp]);
        mark("decoder_block"+std::to_string(i));
    }
    auto x=pixel_head_.forward(z);
    mark("renderer_latent_embedding");
    auto modulation=dec_cond_embed_.forward_dense_conv1x1(s);
    mark("renderer_condition_projection");
    x=res_block_.forward(x,modulation,resb_scale_[qp]);
    mark("renderer_adaln_mlp_residual");
    auto rgb=final_layer_.forward(x);
    mark("renderer_rgb_projection");
    return times;
}
