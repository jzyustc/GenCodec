...
at::Tensor DenseConv1x1Bf16::forward_dense_conv1x1_scale_decoder(const at::Tensor& feature,
                                                                 float qp_bias) const
{
    TORCH_CHECK(input_channels_ == 144 && output_channels_ == 5120 && upscale_factor_ == 4
                    && output_chunks_ == 1 && !fuse_silu_ && !fuse_hard_gelu_030_ && !fuse_sigmoid_
                    && bias_.has_value(),
                "scale decoder path requires a plain biased 144->5120 pixel-shuffle convolution");
...
{
    TORCH_CHECK(input_channels_ == 960 && output_channels_ == 256 && upscale_factor_ == 1
                    && output_chunks_ == 1 && !fuse_silu_ && !fuse_hard_gelu_030_ && !fuse_sigmoid_
                    && bias_.has_value(),
                "three-input dense path requires plain biased 960->256 convolution");
    at::Tensor output =
...
{
    TORCH_CHECK(input_channels_ == 96 && output_channels_ == 96 && upscale_factor_ == 1
                    && output_chunks_ == 1 && !fuse_silu_ && !fuse_hard_gelu_030_ && !fuse_sigmoid_
                    && bias_.has_value(),
                "gated residual dense path requires plain biased 96->96 convolution");
...
{
    TORCH_CHECK(input_channels_ == 48 && output_channels_ == 96 && upscale_factor_ == 1
                    && output_chunks_ == 1 && !fuse_silu_ && !fuse_hard_gelu_030_ && !fuse_sigmoid_
                    && !bias_.has_value(),
                "periodic bias dense path requires plain unbiased 96->96 convolution");
    at::Tensor output =
...
{
    TORCH_CHECK(input_channels_ == 96 && output_channels_ == 288 && upscale_factor_ == 1
                    && output_chunks_ == 3 && !fuse_silu_ && !fuse_hard_gelu_030_ && !fuse_sigmoid_
                    && bias_.has_value(),
                "Ada affine path requires a plain biased 96->288 three-chunk convolution");
...
{
    TORCH_CHECK(input_channels_ == 256 && output_channels_ == 320 && upscale_factor_ == 1
                    && output_chunks_ == 1 && !fuse_silu_ && !fuse_hard_gelu_030_ && !fuse_sigmoid_
                    && bias_.has_value(),
                "spatial reconstruction path requires a plain biased 256->320 convolution");
...
{
    TORCH_CHECK(input_channels_ == 256 && output_channels_ == 640 && upscale_factor_ == 1
                    && output_chunks_ == 2 && !fuse_silu_ && !fuse_hard_gelu_030_ && !fuse_sigmoid_
                    && bias_.has_value(),
                "prior masked-add path requires a plain biased 256->640 two-chunk convolution");
...
DepthwiseConv3x3Bf16::forward_depthwise_conv3x3_hard_gelu_030_mean(const at::Tensor& feature) const
{
    std::tuple<at::Tensor, at::Tensor> output =
        depthwise_conv3x3_bias_hard_gelu_030_mean_avx512_bf16(feature, packed_weight_, bias_);
    return output;
}
