...
static inline __m512bh load_group_input_pairs(const at::BFloat16* input, int pair)
{
    ...
    // Construct the four group pairs once, then explicitly broadcast the
    // 128-bit pattern. A 16-argument set_epi32 here makes GCC materialize the
    // repeated pattern through scalar stack updates in the reduction loop.
    // The lane bits and subsequent BF16 accumulation order are unchanged.
    return (__m512bh)_mm512_broadcast_i32x4(_mm_set_epi32(pair3, pair2, pair1, pair0));
}
                                                    ...
                                                    const at::optional<at::Tensor>& output_scale)
{
    ...
    DISPATCH_PAIR(288, 144)
    DISPATCH_PAIR(256, 256)
    DISPATCH_PAIR(512, 256)
