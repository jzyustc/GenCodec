...
void DiCoBlockCAProxy::set_param(const std::map<std::string, at::Tensor>& sd)
{
    ...
    ca_.set_param(sd.at("ca.1.weight"), sd.at("ca.1.bias"), 1, 1, false, false, true);
}

at::Tensor DiCoBlockCAProxy::forward(const at::Tensor& inp, const at::optional<at::Tensor>& scale) const
{
    at::Tensor x = conv1_.forward_dense_conv1x1(inp);
    at::Tensor pooled;
    std::tie(x, pooled) = conv2_.forward_depthwise_conv3x3_hard_gelu_030_mean(x);
    const at::Tensor gate = ca_.forward_dense_conv1x1(pooled);
    x = conv3_.forward_dense_conv1x1(x, gate, inp);
    at::Tensor t = conv4_.forward_dense_conv1x1(x);
    at::Tensor out = conv5_.forward_dense_conv1x1(t, at::nullopt, x, scale);
    return out;
}

void EncoderDiCoBlockCAProxy::set_param(const std::map<std::string, at::Tensor>& sd, int64_t groups)
{
...
at::Tensor EncoderDiCoBlockCAProxy::forward(const at::Tensor& inp,
                                            const at::optional<at::Tensor>& scale) const
{
    ...
    std::tie(x, pooled) = conv2_.forward_depthwise_conv3x3_hard_gelu_030_mean(x);
    const at::Tensor gate = ca_.forward_dense_conv1x1(pooled);
    x = conv3_.forward_grouped_conv1x1(x, gate, inp);
