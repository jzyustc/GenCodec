        ...
        for (int64_t y = 0; y < height; ++y) {
            ...
            if constexpr (tail != 0) {
                depthwise_conv3x3_channel_block<CHANNELS, true>(
                    feature, packed_weight, bias, output, batch_index, y, height, width,
                    full_blocks * CHANNEL_BLOCK);
            }
        }
    }
}

static at::Tensor depthwise_conv3x3_hard_gelu_030_impl(
...
{
    return depthwise_conv3x3_hard_gelu_030_impl(
        feature,
        packed_weight,
        bias);
}
