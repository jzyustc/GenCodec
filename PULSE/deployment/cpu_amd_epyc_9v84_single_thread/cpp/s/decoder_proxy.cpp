                                   ...
                                   int64_t mask_width)
{
    ...
    TORCH_CHECK(mask_channels == g_y_channels,
                "PULSE-S decoder requires M=", g_y_channels, ", got ", mask_channels);
    TORCH_CHECK(sd.at("entropy_model.q_scale_hyper_z").size(1) == 1,
                "PULSE-S hyper-z QP scale must be scalar");
    TORCH_CHECK(sd.at("entropy_model.hyper_dec.body.0.up.weight").size(1) == g_z_channels,
                "PULSE-S decoder requires z_ch=", g_z_channels);
    TORCH_CHECK(sd.at("entropy_model.em_step1.in_proj.bias").size(0)
                    == g_entropy_inner_channels,
                "PULSE-S EM step-1 width mismatch");
    TORCH_CHECK(sd.at("entropy_model.em_step2.in_proj.bias").size(0)
                    == g_entropy_inner_channels,
                "PULSE-S EM step-2 width mismatch");
    TORCH_CHECK(sd.at("decoder.up0.0.bias").size(0) == g_decoder_body_channels,
                "PULSE-S decoder body width mismatch");
    TORCH_CHECK(sd.at("decoder.renderer.input_proj.weight").size(0)
                    == g_renderer_channels,
                "PULSE-S renderer width mismatch");
    TORCH_CHECK(sd.at("decoder.renderer.input_proj.weight").size(1)
                    == g_renderer_bottleneck_channels,
                "PULSE-S renderer bottleneck width mismatch");
                                               ...
                                               const at::Tensor& compact1_nhwc, int64_t qp) const
{
    TORCH_CHECK(z_i16.device().is_cpu() && z_i16.scalar_type() == at::kShort
                    && z_i16.dim() == 4 && z_i16.size(1) == g_z_channels
                    && z_i16.is_contiguous(),
