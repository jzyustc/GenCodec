...
{
    // Combined calibrated streams map part1's raw scale rows to64..127.
    // The caller shifts the cutoff by64 as well; the raw scale cutoff
    // remains0..63. Split coders keep their local64-row tables unchanged.
    if (cutoff < 0 || cutoff >= 128) {
        throw std::runtime_error("rANS y row cutoff must be in [0, 127]");
    }
}
...
py::array_t<int16_t> RansDecoder::get_decoded_tensor()
{
    ...
    // Without a base owner pybind11 copies the pointer data into an owning array.
    // Preserve this historical API's stable-copy semantics.
    return py::array_t<int16_t>(m_current_decoded_tensor_size, decoded->data());
}

py::array_t<int16_t> RansDecoder::get_decoded_tensor_view()
{
    auto decoded = get_decoded_tensor_cpp();
    if (!decoded) {
        throw std::runtime_error("no decoded buffer is available");
    }
    if (m_current_decoded_tensor_size == 0) {
        return py::array_t<int16_t>(0);
    }
    // Keep old storage alive even if a later decode reallocates the buffer or
    // the Python decoder is destroyed. Consume before the next decode reuses it.
    using Owner = std::shared_ptr<PinnedHostBuffer<int16_t>>;
    py::capsule owner(new Owner(decoded), [](void* pointer) {
        delete static_cast<Owner*>(pointer);
    });
    return py::array_t<int16_t>(
        {static_cast<py::ssize_t>(m_current_decoded_tensor_size)},
