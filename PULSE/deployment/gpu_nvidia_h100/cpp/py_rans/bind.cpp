...
PYBIND11_MODULE(pulse_gpu_entropy_ext, m)
{
    m.def("offset_cdf_symbols", &offset_cdf_symbols);
    m.def("offset_cdf_indexes", &offset_cdf_indexes);
    py::class_<MetaPriorSelector>(m, "MetaPriorSelector", py::module_local())
        .def(py::init<
             int,
             int,
    ...
    py::class_<LogIndexDecoder>(m, "LogIndexDecoder", py::module_local())
        .def(py::init<
             int,
             int,
             int,
    ...
    py::class_<RansDecoder>(m, "RansDecoder", py::module_local())
        ...
        .def("get_decoded_tensor_view", &RansDecoder::get_decoded_tensor_view)
        .def("set_cdf",
             py::overload_cast<const py::array_t<int32_t>&, const py::array_t<int32_t>&, const int>(
                 &RansDecoder::set_cdf))
